...
public:
    TileMapEditor() : tile_map({},
                               nullptr, TextureBank::get("test"), {32, 32}) {
        add_child(tile_map);
        tile_map.load_tiles();
    ...
    void update() override {
        ...
        FontBank::draw(as_ui({0, 0}), edit_mode_str, {.color = Colors::White, .font_size = 16.0f / Camera2D::get_current_zoom()});
    ...
    std::vector<Rect2> get_edit_squares(Vector2 pos, int bits) {
